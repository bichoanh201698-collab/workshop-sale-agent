...
docs/CONTENT.md       ← bản bóc tách nội dung 26 slide từ pptx của khách
...
## Xuất file ảnh / PDF
```bash
for i in $(seq 1 26); do ./shot.sh $i "out/slide-$(printf %02d $i).png"; done   # PNG 4480×1120
```
`shot.sh` mở deck ở chế độ `?export=1` nên ảnh xuất ra không có progress bar hay thanh điều khiển.
Bộ 26 PNG đã render sẵn trong `out/`.
